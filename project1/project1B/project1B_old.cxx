...
int getT(double X[3], double Y[3]) {
...
int getL(double X[3], double Y[3]) {
  int top = getT(X, Y);
 
  if (top != 0 && X[0] <= X[1] && X[0] <= X[2]) return 0;
  if (top != 1 && X[1] <= X[0] && X[1] <= X[2]) return 1;
  if (top != 2 && X[2] <= X[0] && X[2] <= X[1]) return 2;

  return -1;
}

int getR(double X[3], double Y[3]) {
  int top  = getT(X, Y);
  int left = getL(X, Y);

  for (int i = 0; i < 3; i++)
    if (top != i && left != i) return i;
...
int main()
{
  ...
  //for (int i = 0; i < 3; i++) {
    ...
    double *X = triangles[i].X;
    double *Y = triangles[i].Y;
    ...
    Line *lLine = new Line(X[T], Y[T], X[L], Y[L]);
    Line *rLine = new Line(X[T], Y[T], X[R], Y[R]);

    //lLine->printLine();
    //rLine->printLine();

    double rowMin = ceil441(Y[L]);
    double rowMax = floor441(Y[T]);
